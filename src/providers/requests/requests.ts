import {Injectable} from '@angular/core';
import {Events} from 'ionic-angular';
import {addFriendRequest} from '../../models/interfaces/addFriendRequest';
import {UserProvider} from '../user/user';
import firebase from 'firebase';

@Injectable()
export class RequestsProvider {
  firereq = firebase.database().ref('/requests');
  userdetails;
  firefriends = firebase.database().ref('/friends');
  myfriends;
  notfriends;

  constructor(public userservice: UserProvider, public events: Events) {

  }

  acceptrequest(buddy) {
    var promise = new Promise((resolve, reject) => {
      this.myfriends = [];
      this.firefriends.child(firebase.auth().currentUser.uid).push({
        uid: buddy.uid
      }).then(() => {
        this.firefriends.child(buddy.uid).push({
          uid: firebase.auth().currentUser.uid
        }).then(() => {
          this.deleterequest(buddy).then(() => {
            resolve(true);
          })

        })
      })
    })
    return promise;
  }

  deleterequest(buddy) {
    var promise = new Promise((resolve, reject) => {
      this.firereq.child(firebase.auth().currentUser.uid).orderByChild('sender').equalTo(buddy.uid).once('value', (snapshot) => {
        let somekey;
        for (var key in snapshot.val())
          somekey = key;
        this.firereq.child(firebase.auth().currentUser.uid).child(somekey).remove().then(() => {
          resolve(true);
        })
      })
        .then(() => {

        }).catch((err) => {
        reject(err);
      })
    })
    return promise;
  }

  checkRequest(friendRequest: addFriendRequest) {
    var promise = new Promise((resolve, reject) => {
      this.firereq.child(friendRequest.recipient).once("value", (snapshot) => {
         resolve(snapshot.val());
        }
      );
    });
    return promise;
  }

  sendRequest(friendRequest: addFriendRequest) {
    var promise = new Promise((resolve, reject) => {
      this.firereq.child(friendRequest.recipient).push({
        sender: friendRequest.sender
      }).then(() => {
        resolve({success: true});
      })
    })
    return promise;
  }

  getmyfriends() {
    let friendsuid = [];
    this.firefriends.child(firebase.auth().currentUser.uid).on('value', (snapshot) => {
      let allfriends = snapshot.val();
      this.myfriends = [];
      this.notfriends = [];
      for (var i in allfriends)
        friendsuid.push(allfriends[i].uid);

      this.userservice.getallusers().then((users) => {
        this.myfriends = [];
        for (var key in users) {
          if (friendsuid.indexOf(users[key].uid) >= 0)
            this.myfriends.push(users[key]);
          else
            this.notfriends.push((users[key]));
        }
        this.events.publish('friends');
      }).catch((err) => {
        alert(err);
      })

    })
  }

  getmyrequests() {
    let allmyrequests;
    var myrequests = [];
    this.firereq.child(firebase.auth().currentUser.uid).on('value', (snapshot) => {
      allmyrequests = snapshot.val();
      myrequests = [];
      for (var i in allmyrequests) {
        myrequests.push(allmyrequests[i].sender);
      }
      this.userservice.getallusers().then((res) => {
        var allusers = res;
        this.userdetails = [];
        for (var j in myrequests)
          for (var key in allusers) {
            if (myrequests[j] === allusers[key].uid) {
              this.userdetails.push(allusers[key]);
            }
          }
        this.events.publish('gotrequests');
      })

    })
  }
}
